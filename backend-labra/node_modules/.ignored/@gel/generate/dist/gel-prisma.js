...
exports.runGelPrismaGenerator = runGelPrismaGenerator;
...
function getMod(fullname) {
    return fullname.split("::").slice(0, -1).join("::");
}
function getModAndName(fullname) {
...
async function getSchemaJSON(client) {
    const types = await client.query(INTRO_QUERY);
    const modules = await client.query(MODULE_QUERY);
    return processLinks(types, modules);
}
function _skipInvalidNames(spec_list, recurse_into = null) {
    const valid = [];
...
function processLinks(types, modules) {
    const type_map = {};
    const link_tables = [];
    const link_objects = [];
    const prop_objects = [];
...
function field_name_sort(a, b) {
    const a_match = NAME_RE.exec(a[0]);
    const b_match = NAME_RE.exec(b[0]);
...
class ModelClass {
    ...
    getBacklinkName(src_link, src_name) {
...
class ModelGenerator {
    ...
    renderProp(prop) {
        ...
        const type = GEL_SCALAR_MAP[target];
        if (type === undefined) {
    ...
    renderSingleLink(link, bklink) {
        const opt = link.required ? "" : "?";
        const [mod, target] = getModAndName(link.target.name);
        if (mod !== "default") {
            warn(`Skipping link ${link.name}: link target ${link.target.name} ` +
                `is not supported`);
            return "";
        }
        return (`${target}${opt}    @relation("${bklink}", ` +
            `fields: [${link.name}_id], references: [id], ` +
            `onUpdate: NoAction, onDelete: NoAction)`);
    }
    renderBacklinkLink(link) {
    ...
    renderModels(spec) {
        ...
        if (mods[0] != "default" || mods.length > 1) {
            ...
            warn(`Skipping modules ${skipped}: Prisma reflection doesn't support` +
                "multiple modules or non-default modules.");
        }
        if (spec.prop_objects.length > 0) {
            warn("Skipping multi properties: Prisma reflection doesn't support multi" +
    ...
    renderModelClass(mod) {
        this.write(`model ${mod.name} {`);
        this.indent();
        if (!mod.isLinkTable) {
            this.write('id    String    @id    @default(dbgenerated("uuid_generate_v4()"))' +
                "    @db.Uuid");
            this.write('gel_type_id    String    @default(dbgenerated("uuid_generate_v4()"))' +
...
async function runGelPrismaGenerator(params) {
